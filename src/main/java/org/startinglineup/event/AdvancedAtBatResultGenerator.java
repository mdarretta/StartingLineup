...
/**
 * A "stubbed" engine to predict an outcome for a particular batter/pitcher match up.
 * 
 * This class extends the <code>SimpleAtBatResultsGenerator</code> to account for the
 * propensity for batters to generate particular walks or hits versus a pitcher's 
 * propensity to give up particular walks or hits.
 * 
 * This stubbed engine does not use real data at this time, but only provides randomly
 * generated data to account for pitcher propensity. A future implementation needs 
 * to utilize real data.
 * 
 ...
 */
 ...
 public class AdvancedAtBatResultGenerator extends SimpleAtBatResultGenerator {
	...
	protected int getPredictedNumOutcomes(Outcome outcome) {
		float batterOutcomes = (float) ((1000F / atBat.getBatter().getPlateAppearances()) * atBat.getBatter().getNumOutcomes(outcome));
